<template>
    <AppLayout title="Gestión de Estados de Cita">
        <template #header>
                    ...
                    <div class="mb-6 flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-medium text-gray-900">
                                Estás en la vista: Gestión de Estados
                            </h3>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                >
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">
                                            {{ formatDate(estados.created_at) }}
                                        </div>
                                    </td>
                                    <td
                                        class="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-3"
                                    >
                                        <Link
                                            :href="
                                                route(
                                        ...
                                        <button
                                            @click="deleteestados(estados.id)"
                                            :disabled="
                                                deletingestados.includes(
                                                    estados.id
                                                )
                                            "
...
const deleteestados = (id) => {
    if (confirm("¿Estás seguro de eliminar este estado?")) {
        deletingestados.value.push(id);

        router.delete(route("estados.destroy", id), {
            onFinish: () => {
                deletingestados.value = deletingestados.value.filter(
