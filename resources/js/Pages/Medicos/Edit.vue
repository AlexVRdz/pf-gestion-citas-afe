<template>
          ...
          <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
            <div class="mb-6">
              <h3 class="text-lg font-medium text-gray-900">
                Editando: {{ medico.user.name }} {{ medico.user.apellidos }}
              </h3>
  ...
  const form = useForm({
    user_id: props.medico.user_id,
    especialidad: props.medico.especialidad,
    cedula_profesional: props.medico.cedula_profesional,
    tipo_contrato_id: props.medico.tipo_contrato_id
  });
  ...
  const hasChanges = computed(() =>
    ...
    form.cedula_profesional !== original.cedula_profesional ||
    parseInt(form.tipo_contrato_id) !== parseInt(original.tipo_contrato_id)
  );
  
  const submit = () => {
    form.put(route("medicos.update", props.medico.id));
  };
  </script> 
